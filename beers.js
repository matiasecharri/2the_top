...
//EJ 1 DONE
const clasificadorCervezas = (array, valorAlc) => {
  const mapedBeers = array.map(beer => ({
    name: beer.name,
    abv: beer.abv,
    ibu: beer.ibu,
  }));

  const filteredBeers = mapedBeers.filter(beer => beer.abv < valorAlc);
  return filteredBeers;
};

const cervezasMenoresABV5 = clasificadorCervezas(beers, 5);

//EJ 2 DONE
const cervezasMasAlcoholicas = array => {
  const cervezasOrdenadas = [...array];
  cervezasOrdenadas.sort((a, b) => b.abv - a.abv);

  const cervezasRecortadas = cervezasOrdenadas.splice(0, 10);
  return cervezasRecortadas;
};

const arrayMasAlcoholicas = cervezasMasAlcoholicas(beers);

//EJ 3 DONE
const cervezasMenosAmargas = array => {
  const eliminarCervezasConProblemas = array.filter(beer => beer.ibu !== null);

  const cervezasOrdenadas = [...eliminarCervezasConProblemas];
  cervezasOrdenadas.sort((a, b) => a.ibu - b.ibu);
